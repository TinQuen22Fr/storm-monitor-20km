...
## [2026-08-03] HOTFIX PROD — 4 causes racines du "tout est cassé" après upgrade (PRÊT À PUSHER)
1. **Blitzortung mort (prod ET sandbox, même erreur)** : depuis ~début août, les serveurs Blitzortung redirigent vers leur page d'accueil les clients WebSocket sans en-tête `Origin` (erreur « https://www.blitzortung.org/ isn't a valid URI ») — ce n'était PAS le réseau sandbox ni l'upgrade. Fix : `origin="https://map.blitzortung.org"` dans `websockets.connect()` (lightning.py). Prouvé en live : 331 strikes reçus, 67 en zone, 0 erreur après restart.
2. **requirements.txt pollué (freeze sandbox)** : contenait `emergentintegrations` (404 sur PyPI → vérifié) + ~120 paquets inutiles (litellm, boto3, huggingface…) → `pip install -r` plantait FORCÉMENT sur le Kimsufi → `upgrade.sh --with-deps` mourait en plein milieu. Fix : requirements.txt réécrit, 22 deps réelles uniquement (résolution 100% PyPI public vérifiée par dry-run).
3. **Node 20 < 20.19 incompatible Vite 8** : install.sh installait Node 20 figé ; Vite 8 exige ≥20.19 ou ≥22. Un build raté VIDAIT build/ → site blanc (« l'appli ne se charge plus »). Fix : install.sh → Node 22 ; upgrade.sh → garde-fou version Node AVANT yarn install/build avec message clair (commandes de mise à jour affichées).
